...
async function listJsonSupplementsForMessages(messageIds) {
  if (!messageIds.length) return new Map();
  const supabase = getSupabaseAdmin();
  const { data, error } = await supabase
    .from("messages")
  ...
  if (error) {
    if (error.code === "42703" || /reply_supplements/i.test(error.message || "")) {
      return new Map();
    }
    throw error;
  }
...
async function attachSupplements(messages) {
  ...
  const grouped = await listSupplementsForMessages(ids);
  const jsonGrouped = await listJsonSupplementsForMessages(ids);
...
export async function listPublicMessages() {
  ...
  if (error) throw error;
  return (await attachSupplements(data || [])).map(toPublicMessage);
}
...
export async function createReplySupplement(messageId, content) {
  ...
  if (!error && existingRows?.length) {
    ...
    if (!error && extraIds.length) {
      await supabase.from("reply_supplements").delete().in("id", extraIds);
    }
  } else if (!error) {
  ...
  if (error) {
    ...
    const currentSupplements = Array.isArray(message?.reply_supplements)
      ? message.reply_supplements
      : [];
